...
def place_entities(
    ...
    number_of_monsters = random.randint(0, maximum_monsters) # random num between 0 and max_monsters

    for i in range(number_of_monsters):
        x = random.randint(room.x1 + 1, room.x2 - 1)
        y = random.randint(room.y1 + 1, room.y2 - 1)

        if not any(entity.x == x and entity.y == y for entity in dungeon.entities): #spawn creature if not overlapping
            if random.random() < 0.8:
                entity_spawn.orc.spawn(dungeon, x, y)
            else:
                entity_spawn.troll.spawn(dungeon, x, y)
...
def generate_dungeon( 
    ...
    for r in range(max_rooms):
        ...
        place_entities(new_room, dungeon, max_monsters_per_room) # function call to stick in the npcs.
